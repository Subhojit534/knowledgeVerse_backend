...
function getCanonicalSubject(cat: string): string | null {
  const c = cat.trim().toUpperCase();
  if (c.includes('MATH')) return 'Mathematics';
  if (c.includes('PHYSIC')) return 'Physics';
  if (c.includes('CHEM')) return 'Chemistry';
  if (c.includes('BIO')) return 'Biology';
  if (c.includes('HIST')) return 'History';
  if (c.includes('CS') || c.includes('CODE') || c.includes('COMP')) return 'Computer Science';
  return null;
}
...
async function buildLeaderboard(category: string = 'GLOBAL') {
  ...
  if (catUpper === 'GUILDS') {
    ...
    const sortedGuilds = [...guilds].sort(
      (a, b) => ((b.level || 1) * 1000 + (b.member_count || 1) * 100) - ((a.level || 1) * 1000 + (a.member_count || 1) * 100)
    );
